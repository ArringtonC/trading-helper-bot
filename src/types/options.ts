...
export interface OptionTrade {
  ...
  proceeds?: number;
  // Asset category for filtering (e.g., 'OPT', 'STK', 'CASH')
  assetCategory?: string;

  // --- Fields for ML Analysis ---

  // Greeks (optional, snapshot at open/close or specific times)
  openDelta?: number;
  openGamma?: number;
  openTheta?: number;
  openVega?: number;
  ...
  // Market Conditions (optional, snapshot at open/close)
  underlyingPriceAtOpen?: number;
  underlyingPriceAtClose?: number;
  ivAtOpen?: number; // Implied Volatility
  ivAtClose?: number;
  hvAtOpen?: number; // Historical Volatility (e.g., 30d)
  hvAtClose?: number;
  vixAtOpen?: number;
  vixAtClose?: number;
  // Add other relevant index/indicator values if needed

  // ML Model Outputs (optional)
  marketRegime?: string;
  identifiedPattern?: string;
  sizingRecommendation?: number; // Could be contracts, percent, etc.
