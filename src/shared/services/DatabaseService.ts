...
import { AssetCategory, BrokerType, NormalizedTradeData, OpenCloseIndicator, PutCall, TradeCounts } from '../types/trade';
...
const initializeSqlJs = async (): Promise<SqlJsStatic> => {
  if (sql) {
    return sql;
  }
...
export const getDb = async (): Promise<Database> => {
  console.log('[DEBUG getDb] Attempting to get DB instance...');
  if (!db) {
    console.log('[DEBUG getDb] db is null, initializing...');
    db = initializeSqlJs().then(SQLModule => {
      sql = SQLModule;
      console.log('[DEBUG getDb] SQL.js module initialized. Creating new DB.');
      const existingDb = new sql.Database(); 
      console.log('[DEBUG getDb] New DB created. Running schema setup...');
      // Create comprehensive schema that matches NormalizedTradeData
      existingDb.run(`
      ...
      console.log('[DEBUG getDb] Running database migrations...');
      runMigrations(existingDb);
      
      console.log('[DEBUG getDb] Schema setup and migrations complete.');
      return existingDb;
    });
  }
  return db;
...
export async function clearDatabase(): Promise<void> {
  ...
  // By setting db to null, the next call to getDb() anywhere in the app
  // will trigger a full re-initialization of the database and its schema.
  db = null; 
}
...
export async function resetDatabase() {
  ...
  db = null; // Force re-initialization on next getDb call
  await getDb(); // This will reinitialize the database and create the schema
}
...
export async function insertNormalizedTrades(trades: NormalizedTradeData[]): Promise<{ successCount: number; errors: { tradeId: string | undefined, error: any }[] }> {
  ...
  try {
    ...
    for (const trade of trades) {
      try {
        ...
        if (anomalies.length > 0) {
          ...
          // Skip trades with critical anomalies that would cause insertion failures
          const criticalAnomalies = anomalies.filter(msg => 
            msg.includes('Zero trade price') || 
            msg.includes('Zero quantity') ||
            msg.includes('Missing required fields')
          );
          
          if (criticalAnomalies.length > 0) {
            console.warn(`[SKIP] Skipping trade due to critical anomalies:`, trade);
            continue;
          }
        }
        ...
        // Debug: Log key trade values to understand issues
        console.log(`[DEBUG] Trade values - Price: ${trade.tradePrice}, Quantity: ${trade.quantity}, Symbol: ${trade.symbol}, NetAmount: ${trade.netAmount}`);
    ...
    // Always commit successful insertions, even if some trades had errors
    currentDb.exec("COMMIT;");
    
    if (insertionErrors.length > 0) {
      console.warn(`Successfully inserted ${successCount} trades, but ${insertionErrors.length} trades had errors:`, insertionErrors);
    } else {
      console.log(`Successfully inserted all ${successCount} trades.`);
    }
  } catch (e: any) {
