  ...
  static getOptionsPortfolio(accountId: string): OptionsPortfolio {
    ...
    if (normalizedAccountId === 'demo1' && !portfolios[normalizedAccountId]) {
      ...
      const demoPortfolio = {
        ...
        cumulativePL: SAMPLE_TRADES.reduce((total, trade) => total + (trade.tradePL || 0), 0) // Calculate from trades
      };
      this.savePortfolio(demoPortfolio);
      console.log('Returning demo portfolio with cumulativePL:', demoPortfolio.cumulativePL);
    ...
    console.log('Returning portfolio with cumulativePL:', portfolio.cumulativePL);
    return portfolio;
  }
