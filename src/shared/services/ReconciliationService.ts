...
export interface ReconciliationInput {
  sourceA: DailyPnlData[]; // Data from local DB
  sourceB: DailyPnlData[]; // Data from authoritative source
}

export type DiscrepancyType = 'Missing_SourceA' | 'Missing_SourceB' | 'Pnl_Mismatch';

export interface ReconciliationDiscrepancy {
  symbol: string;
  date: string;
  type: DiscrepancyType;
  pnlSourceA: number | null;
  pnlSourceB: number | null;
  difference: number | null;
}

export interface ReconciliationResult {
  reconciledItems: DailyPnlData[];
  ...
  summary: {
    ...
    countMissingSourceA: number;
    countMissingSourceB: number;
    countPnlMismatch: number;
  };
}
 ...
 */
export function reconcilePnlData(
  input: ReconciliationInput,
  tolerance: number = DEFAULT_TOLERANCE
): ReconciliationResult {
  const mapA = new Map<string, DailyPnlData>();
  const mapB = new Map<string, DailyPnlData>();
  ...
  const reconciledItems: DailyPnlData[] = [];
