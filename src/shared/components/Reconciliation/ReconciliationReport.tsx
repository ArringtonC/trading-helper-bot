...
import { ReconciliationResult, ReconciliationDiscrepancy } from '../../services/ReconciliationService';
...
const getDiscrepancyTypeText = (type: ReconciliationDiscrepancy['type']) => {
  switch (type) {
...
const ReconciliationReport: React.FC<ReconciliationReportProps> = ({ result, isLoading, error }) => {
  ...
  return (
    <div className="p-4 border rounded shadow-md bg-white mt-4 space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
        ...
        <div className={`p-2 rounded ${summary.countDiscrepancies > 0 ? 'bg-red-100 text-red-800' : 'bg-gray-100'}`}><span className="font-medium">Total Discrepancies:</span> {summary.countDiscrepancies}</div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {discrepancies.map((item, index) => (
                  <tr key={`${item.symbol}-${item.date}-${index}`} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    <td className={`px-4 py-2 whitespace-nowrap text-right ${item.pnlSourceA === null ? 'text-gray-400' : ''}`}>{formatPnl(item.pnlSourceA)}</td>
                    <td className={`px-4 py-2 whitespace-nowrap text-right ${item.pnlSourceB === null ? 'text-gray-400' : ''}`}>{formatPnl(item.pnlSourceB)}</td>
                    <td className={`px-4 py-2 whitespace-nowrap text-right font-medium ${item.difference && Math.abs(item.difference) > 0.01 ? 'text-red-600' : 'text-gray-500'}`}>{formatPnl(item.difference)}</td>
      ...
      {summary.countDiscrepancies === 0 && (
         <p className="text-green-600 font-medium">✅ All items reconciled successfully!</p>
      )}
    </div>
  );
};
