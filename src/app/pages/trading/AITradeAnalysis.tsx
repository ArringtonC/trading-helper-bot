...
import { Bar, BarChart, CartesianGrid, Bar as InsightsBar, BarChart as InsightsBarChart, CartesianGrid as InsightsCartesianGrid, ResponsiveContainer as InsightsResponsiveContainer, Tooltip as InsightsTooltip, XAxis as InsightsXAxis, YAxis as InsightsYAxis, ResponsiveContainer, Tooltip, XAxis, YAxis } from 'recharts';
...
import { predictHMMRegimes, PredictRequest, PredictResponse, trainHMMModel, TrainRequest } from '../../../shared/services/HMMService'; // Import HMM service functions and types
import { calculateCurrentVolatilityRegime, VolatilityRegime } from '../../../shared/services/MarketAnalysisService';
...
function TradeDetailModal({ open, onClose, trade, features }: { open: boolean, onClose: () => void, trade: any, features: any }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40" role="dialog" aria-modal="true" aria-label="Trade details">
      <div ref={modalRef} tabIndex={-1} className="bg-white rounded shadow-lg p-6 max-w-lg w-full relative outline-none">
        <button className="absolute top-2 right-2 text-gray-500 hover:text-gray-700" onClick={onClose} aria-label="Close details">×</button>
        <h3 className="text-lg font-semibold mb-2">Trade Details</h3>
        <div className="mb-2 text-sm text-gray-700">
          <div><b>ID:</b> {trade.id}</div>
          <div><b>Symbol:</b> {trade.symbol}</div>
          <div><b>Status:</b> {trade.status}</div>
          <div><b>P&L:</b> {trade.tradePL}</div>
          <div><b>Streak:</b> {features?.streak}</div>
          <div><b>Market Regime:</b> {features?.marketRegime}</div>
        </div>
        <h4 className="font-semibold mt-4 mb-1">Feature Breakdown</h4>
        <table className="min-w-full text-xs border">
          <tbody>
            {features && Object.entries(features).map(([k, v]) => (
              <tr key={k}><td className="border px-2 py-1 font-medium">{k}</td><td className="border px-2 py-1">{String(v)}</td></tr>
            ))}
          </tbody>
        </table>
      </div>
...
const AITradeAnalysis: React.FC = () => {
  ...
  const [currentMarketRegime, setCurrentMarketRegime] = useState<VolatilityRegime>(VolatilityRegime.UNKNOWN);
  const [regimeHistory, setRegimeHistory] = useState<{ date: string; regimeValue: number; regimeLabel: string }[]>([]);

  // State for HMM inputs
  const [hmmSymbol, setHmmSymbol] = useState('SPY');
  const [hmmStartDate, setHmmStartDate] = useState('2024-12-01'); // Default to match CSV date range
  const [hmmEndDate, setHmmEndDate] = useState('2024-12-30');   // Default to match CSV date range
  const [hmmMarketDataFile, setHmmMarketDataFile] = useState<File | null>(null);
  const [includeVix, setIncludeVix] = useState(false); // VIX feature toggle
  ...
  const [hmmPredictedRegimes, setHmmPredictedRegimes] = useState<PredictResponse | null>(null);

  // State for downloaded data preview
  const [downloadedData, setDownloadedData] = useState<{ source: string; data: any[]; filename: string; dateRange?: any } | null>(null);
  
  // State for managing multiple datasets
  const [spyData, setSpyData] = useState<{ data: string; filename: string; dateRange?: any } | null>(null);
  const [vixData, setVixData] = useState<{ data: string; filename: string; dateRange?: any } | null>(null);
  const [combinedData, setCombinedData] = useState<{ data: string; filename: string } | null>(null);

  // New state for market data file path
  const marketDataFileInputRef = useRef<HTMLInputElement>(null);

  const [hmmServiceStatus, setHmmServiceStatus] = useState<'checking' | 'connected' | 'not_running'>('checking');
  ...
  const winZoneTrades = useMemo(() => features.filter(f => typeof f.streak === 'number' && f.streak >= 2), [features]);
  ...
  useEffect(() => {
    ...
    fetch('/data/features.json').then(r => r.json()).then(data => {
      if (Array.isArray(data) && data.length > 0 && data[0].featureImportances) {
    ...
    const loadTradesAndAnalyseRegime = async () => {
      try {
        ...
        const priceSeriesForRegime = formattedTrades
          .filter(t => t.status === 'Closed' && t.closeDate != null && typeof t.closePremium === 'number')
          .map(t => ({ date: new Date(t.closeDate!), price: t.closePremium! }))
          .sort((a, b) => a.date.getTime() - b.date.getTime());

        if (priceSeriesForRegime.length > 0) {
          // Calculate current regime
          const currentRegimeCalc = calculateCurrentVolatilityRegime(priceSeriesForRegime);
          setCurrentMarketRegime(currentRegimeCalc);

          // Calculate regime history (last 30 data points)
          const historyPoints = 30;
          const regimeConfig = { windowSize: 20, lowPercentile: 25, highPercentile: 75, minDataForPercentile: 30 };
          const calculatedHistory: { date: string; regimeValue: number; regimeLabel: string }[] = [];
          
          // Ensure we have enough data for at least one calculation plus history
          const startCalcIndex = Math.max(regimeConfig.windowSize, priceSeriesForRegime.length - historyPoints);
          ...
          for (let k = startCalcIndex; k < priceSeriesForRegime.length; k++) {
            ...
            if (historicalPricesUpToK.length >= regimeConfig.windowSize + 1) {
              const regimeAtK = calculateCurrentVolatilityRegime(historicalPricesUpToK, regimeConfig);
  ...
  const handleMarketDataFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleTrainHMM = async () => {
    ...
    try {
      ...
      setHmmError(`Training failed: ${error.message}`);
  ...
  // Handler for VIX evaluation
  const handleEvaluateVIX = async () => {
    ...
    try {
      ...
      const response = await fetch(`${HMM_BACKEND_URL}/evaluate`, {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        ...
        console.log('VIX evaluation completed:', data);
      } else {
        throw new Error(data.error || 'Unknown evaluation error');
      }
    } catch (error) {
      console.error('Error running VIX evaluation:', error);
      setHmmError(`Evaluation failed: ${error}`);
    } finally {
  ...
  const handlePredictHMM = async () => {
    ...
    // Calculate future prediction dates (next 2 weeks from today)
    const today = new Date();
    const twoWeeksFromNow = new Date(today);
    twoWeeksFromNow.setDate(today.getDate() + 14);
    
    const futureStartDate = today.toISOString().split('T')[0]; // Today as YYYY-MM-DD
    const futureEndDate = twoWeeksFromNow.toISOString().split('T')[0]; // 2 weeks from now
    
    console.log(`🔮 Predicting future regimes from ${futureStartDate} to ${futureEndDate}`);

    const requestBody: PredictRequest = {
      symbol: hmmSymbol,
      startDate: futureStartDate, // Future start date
      endDate: futureEndDate,     // Future end date (2 weeks ahead)
      trade_data: csvString,
      market_data_filepath: hmmMarketDataFile ? hmmMarketDataFile.name : undefined,
    ...
    try {
      const response = await predictHMMRegimes(requestBody);
      setHmmPredictedRegimes(response);
      console.log('HMM Prediction successful:', response);
    } catch (error: any) {
      console.error('Failed to predict HMM regimes:', error);
      setHmmError(error.message || 'An error occurred during prediction.');
    } finally {
      setHmmLoading(false);
    }
  };
  ...
  const summaryMetrics = useMemo((): TradeSummaryMetrics | null => {
    ...
    const closedTrades = trades.filter(t => t.status === 'Closed' && t.tradePL !== undefined);
  ...
  const combineSpyAndVixData = () => {
    if (!spyData || !vixData) {
      alert('Please download both SPY and VIX data first before combining.');
      return;
    }
    ...
    try {
      ...
      for (let i = 1; i < spyRows.length; i++) {
        ...
        if (spyCols.length >= 2 && spyCols[0].trim()) {
          ...
          if (vixCols && vixCols.length > 0 && !vixCols[0].includes('undefined')) {
    ...
    try {
      if ((window as any).electronAPI?.downloadIBKRVIXData) {
        ...
        if (result.success && result.data) {
          ...
          console.log('Raw VIX CSV data preview:', result.data.substring(0, 500));
          ...
          alert(`VIX data downloaded successfully from ${result.source}! ${result.dataPoints} data points from ${result.dateRange?.start} to ${result.dateRange?.end}`);
        } else {
          console.error('VIX download failed:', result.error);
          alert(`Failed to download VIX data: ${result.error}`);
        }
      } else {
        alert('VIX download function not available. Please ensure IBKR is connected.');
      }
    } catch (error) {
  ...
  const handleDownloadSpyDataIBKR = async () => {
    try {
      if ((window as any).electronAPI?.downloadIBKRHistoricalData) {
        ...
        const result = await (window as any).electronAPI.downloadIBKRHistoricalData({
          symbol: 'SPY',
          duration: '2 Y', // 2 years of data
          barSize: '1 day' // Daily bars
        });
        ...
        if (result.success && result.data) {
          ...
          alert(`SPY data downloaded successfully from ${result.source}! ${result.dataPoints} data points from ${result.dateRange?.start} to ${result.dateRange?.end}`);
        } else {
          console.error('IBKR download failed:', result);
          alert(`Failed to download SPY data from IBKR: ${result.error || 'Unknown error'}`);
        }
      } else {
        alert('IBKR download not available. Make sure Electron is running and IBKR is connected.');
      }
    } catch (error) {
  ...
  const handleDownloadSpyDataAlphaVantage = async () => {
    try {
      if ((window as any).electronAPI?.downloadAlphaVantageData) {
        console.log('Using Electron API to download SPY data from Alpha Vantage...');
        const result = await (window as any).electronAPI.downloadAlphaVantageData({
          symbol: 'SPY',
        ...
        if (result.success && result.data) {
          ...
          alert(`SPY data downloaded successfully from ${result.source}! Historical data through ${result.dateRange?.end}`);
        } else {
          console.error('Alpha Vantage download failed:', result.error);
          alert(`Failed to download SPY data from Alpha Vantage: ${result.error || 'Unknown error'}`);
        }
      } else {
        alert('Alpha Vantage download not available. Make sure Electron is running.');
      }
    } catch (error) {
      console.error('Error downloading SPY data from Alpha Vantage:', error);
      alert('Error downloading SPY data from Alpha Vantage. Check console for details.');
    }
  };
    ...
    try {
      ...
      if ((window as any).electronAPI?.downloadSpyData) {
        ...
        if (result.success && result.data) {
          ...
          alert(`SPY data downloaded successfully! Range: ${result.dateRange?.start} to ${result.dateRange?.end}`);
        } else {
          console.error('Download failed:', result.error);
          alert(`Failed to download SPY data: ${result.error || 'Unknown error'}`);
        }
      } else {
        // Fallback to fetch (will still have CORS issues in browser)
        console.warn('Electron API not available, falling back to fetch (may have CORS issues)');
        ...
        saveAs(blob, `SPY_${startDate.toISOString().split('T')[0]}_to_${endDate.toISOString().split('T')[0]}.csv`);
        alert(`SPY data downloaded successfully! Range: ${startDate.toISOString().split('T')[0]} to ${endDate.toISOString().split('T')[0]}`);
      }
    } catch (error) {
  ...
  const filteredFeatures = features
    .filter(f => !insightsFilter || (f.symbol && f.symbol.toLowerCase().includes(insightsFilter.toLowerCase())))
    .sort((a, b) => insightsSort === 'asc' ? (a.streak ?? 0) - (b.streak ?? 0) : (b.streak ?? 0) - (a.streak ?? 0));
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center space-x-4 mb-2">
        ...
        {hmmServiceStatus === 'checking' && <span className="px-2 py-1 rounded bg-gray-200 text-gray-700">Checking...</span>}
        {hmmServiceStatus === 'connected' && <span className="px-2 py-1 rounded bg-green-200 text-green-800">🟢 Connected</span>}
        {hmmServiceStatus === 'not_running' && <span className="px-2 py-1 rounded bg-red-200 text-red-800">🔴 Not Running</span>}
        <button onClick={() => setShowTroubleshooting(v => !v)} className="ml-4 text-blue-600 underline text-sm">Troubleshooting</button>
      </div>
      {hmmServiceStatus === 'not_running' && (
        <div className="p-3 border rounded bg-yellow-50 text-yellow-900 mb-4">
          <p className="mb-2">HMM Service is not running. To enable ML analysis, open a terminal and run:</p>
          <div className="flex items-center space-x-2">
            <code className="bg-gray-100 px-2 py-1 rounded">cd hmm-service &amp;&amp; python3 app.py</code>
            <CopyToClipboard text="cd hmm-service &amp;&amp; python3 app.py">
              <button className="px-2 py-1 bg-blue-500 text-white rounded text-xs">Copy Command</button>
            </CopyToClipboard>
          </div>
        </div>
      )}
      {showTroubleshooting && (
        <div className="p-3 border rounded bg-gray-50 text-gray-800 mb-4">
          ...
          <ul className="list-disc pl-5 text-sm mb-2">
            ...
            <li>Run <code>cd hmm-service &amp;&amp; python3 app.py</code> to start the backend.</li>
                            <li>Wait for the message: <code>Running on http://127.0.0.1:5001/</code></li>
            <li>Refresh this page after starting the service.</li>
          </ul>
          <p className="text-xs text-gray-500">For more help, see <code>docs/hmm-service-setup.txt</code>.</p>
        </div>
      )}
      ...
      <div className="p-4 border rounded shadow-sm bg-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4"> {/* Grid for inputs */}
          ...
          <div className="flex items-center">
            ...
            />
            <label htmlFor="includeVix" className="ml-2 block text-sm font-medium text-gray-700">
              📊 Include VIX Features
            </label>
            <div className="ml-2 text-xs text-gray-500">
              (Enhances volatility regime detection)
            </div>
          </div>
          <div className="md:col-span-3"> {/* Span across columns for full width */}
             ...
             <div className="flex items-center space-x-2 mt-1">
               <button
                 type="button"
                 onClick={() => marketDataFileInputRef.current?.click()}
                 className="inline-flex justify-center py-2 px-4 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
               >
                 Choose File
               </button>
               <span className="text-sm text-gray-500 truncate">
                 {hmmMarketDataFile ? hmmMarketDataFile.name : combinedData ? `📊 ${combinedData.filename} (use combined data)` : 'No file chosen'}
               </span>
               <button
                 type="button"
                 onClick={handleDownloadSpyDataIBKR}
                 className="inline-flex justify-center py-2 px-4 border border-indigo-500 shadow-sm text-sm font-medium rounded-md text-indigo-700 bg-indigo-50 hover:bg-indigo-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                 title="Download SPY data from IBKR (PROFESSIONAL, requires IBKR connection, institutional-grade data)"
               >
               ...
               <button
                 ...
                 className="inline-flex justify-center py-2 px-4 border border-orange-500 shadow-sm text-sm font-medium rounded-md text-orange-700 bg-orange-50 hover:bg-orange-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500"
                 title="Download VIX (Volatility Index) data from IBKR (PROFESSIONAL, requires IBKR connection, official CBOE data)"
               >
               ...
               <button
                 ...
                 className="inline-flex justify-center py-2 px-4 border border-purple-500 shadow-sm text-sm font-medium rounded-md text-purple-700 bg-purple-50 hover:bg-purple-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500"
                 title="Download SPY data from Alpha Vantage (RELIABLE, requires API key, full historical data)"
               >
               ...
               <button
                 ...
                 className="inline-flex justify-center py-2 px-4 border border-green-500 shadow-sm text-sm font-medium rounded-md text-green-700 bg-green-50 hover:bg-green-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500"
                 title="Download current SPY data from Yahoo Finance (FREE, includes today's data)"
               >
               ...
               <button
                 ...
                 className="inline-flex justify-center py-2 px-4 border border-blue-500 shadow-sm text-sm font-medium rounded-md text-blue-700 bg-blue-50 hover:bg-blue-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                 title="Download SPY historical data from Polygon.io as CSV (limited to Dec 2024)."
               >
               ...
               <input
                 ...
                 onChange={handleMarketDataFileChange}
                 className="hidden"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 p-3 bg-gray-50 rounded">
          <div className="text-center">
            <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${spyData ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-500'}`}>
              {spyData ? '✅ SPY Data Ready' : '⏳ SPY Data Needed'}
            </div>
            {spyData && <div className="text-xs text-gray-600 mt-1">{spyData.filename}</div>}
          </div>
          <div className="text-center">
            <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${vixData ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-500'}`}>
              {vixData ? '✅ VIX Data Ready' : '⏳ VIX Data Needed'}
            </div>
            {vixData && <div className="text-xs text-gray-600 mt-1">{vixData.filename}</div>}
          </div>
          <div className="text-center">
            <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${combinedData ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-500'}`}>
              {combinedData ? '✅ Combined Dataset Ready' : '⏳ Combined Dataset'}
            </div>
            {combinedData && <div className="text-xs text-gray-600 mt-1">{combinedData.filename}</div>}
          </div>
        </div>

        {/* Data Combination Button */}
        {spyData && vixData && !combinedData && (
          <div className="mb-4 p-3 bg-blue-50 rounded border border-blue-200">
            <div className="flex items-center justify-between">
              <div>
                <h4 className="font-medium text-blue-900">Enhanced Training Available!</h4>
                <p className="text-sm text-blue-700">Combine SPY and VIX data for better volatility regime detection</p>
              </div>
              <button
                onClick={combineSpyAndVixData}
        ...
        <div className="flex space-x-4"> {/* Flex container for buttons */}
          <button
            ...
            disabled={hmmLoading} // Disable while loading
          ...
          >
            {hmmLoading && hmmTrainStatus !== null ? 'Predicting...' : '🔮 Predict Next 2 Weeks'} {/* Update button text based on loading/status */}
          </button>
          <button
            onClick={handleEvaluateVIX}
            disabled={isEvaluating || hmmLoading}
          ...
          >
            {isEvaluating ? 'Evaluating...' : '📊 Evaluate VIX Impact'}
          </button>
        </div>

        {/* HMM Status and Results Area */}
        {(hmmLoading || hmmError || hmmTrainStatus || hmmPredictedRegimes || isEvaluating || evaluationResults) && (
          <div className="mt-4 p-3 border rounded bg-gray-50">
            {hmmLoading && <p className="text-blue-600">Loading HMM results...</p>}
            {isEvaluating && <p className="text-purple-600">Evaluating VIX impact on model performance...</p>}
            ...
            {evaluationResults && evaluationResults.success && (
              <div className="mt-4 p-4 border rounded bg-white">
                <h3 className="text-lg font-semibold mb-3">📊 VIX Integration Evaluation Results</h3>
                
                {/* Overall Improvement Summary */}
                <div className="mb-4 p-3 bg-blue-50 rounded">
                  <h4 className="font-semibold text-blue-900 mb-2">Overall Performance Impact</h4>
                  <div className="text-2xl font-bold text-blue-700">
                    {(evaluationResults.evaluation_summary?.overall_improvement || 0) > 0 ? '+' : ''}
                    {(evaluationResults.evaluation_summary?.overall_improvement || 0).toFixed(2)}% Improvement
                  </div>
                  <p className="text-sm text-blue-600 mt-1">
                    {evaluationResults.evaluation_summary?.statistical_significance?.is_significant 
                ...
                {evaluationResults.evaluation_summary?.metrics_comparison && (
                  <div className="mb-4">
                    <h4 className="font-semibold mb-2">Performance Metrics Comparison</h4>
                    <div className="overflow-x-auto">
                      <table className="min-w-full text-sm border">
                        <thead className="bg-gray-50">
                          <tr>
                            <th className="border px-3 py-2 text-left">Metric</th>
                            <th className="border px-3 py-2 text-center">Without VIX</th>
                            <th className="border px-3 py-2 text-center">With VIX</th>
                            <th className="border px-3 py-2 text-center">Improvement</th>
                          </tr>
                        </thead>
                        <tbody>
                          {evaluationResults.evaluation_summary.metrics_comparison?.no_vix && 
                           Object.entries(evaluationResults.evaluation_summary.metrics_comparison.no_vix).map(([metric, noVixValue]) => {
                            const withVixValue = evaluationResults.evaluation_summary.metrics_comparison.with_vix[metric];
                            const improvement = evaluationResults.evaluation_summary.metrics_comparison.improvement[metric];
                            const noVixNum = typeof noVixValue === 'number' ? noVixValue : 0;
                            const withVixNum = typeof withVixValue === 'number' ? withVixValue : 0;
                            const improvementNum = typeof improvement === 'number' ? improvement : 0;
                            return (
                              <tr key={metric} className="hover:bg-gray-50">
                                <td className="border px-3 py-2 font-medium capitalize">{metric.replace('_', ' ')}</td>
                                <td className="border px-3 py-2 text-center">{noVixNum.toFixed(4)}</td>
                                <td className="border px-3 py-2 text-center">{withVixNum.toFixed(4)}</td>
                                <td className={`border px-3 py-2 text-center font-medium ${improvementNum > 0 ? 'text-green-600' : improvementNum < 0 ? 'text-red-600' : 'text-gray-600'}`}>
                                  {improvementNum > 0 ? '+' : ''}{improvementNum.toFixed(2)}%
                                </td>
                              </tr>
                            );
                          })}
                ...
                {evaluationResults.evaluation_summary?.cross_validation_summary && (
                  <div className="mb-4">
                    <h4 className="font-semibold mb-2">Cross-Validation Results</h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="p-3 bg-gray-50 rounded">
                        <h5 className="font-medium text-gray-700">Without VIX</h5>
                        <div className="text-sm">
                          <div>Accuracy: {evaluationResults.evaluation_summary.cross_validation_summary.no_vix?.accuracy?.mean?.toFixed(4)} (±{evaluationResults.evaluation_summary.cross_validation_summary.no_vix?.accuracy?.std?.toFixed(4)})</div>
                          <div>F1 Score: {evaluationResults.evaluation_summary.cross_validation_summary.no_vix?.f1_score?.mean?.toFixed(4)} (±{evaluationResults.evaluation_summary.cross_validation_summary.no_vix?.f1_score?.std?.toFixed(4)})</div>
                        </div>
                      </div>
                      <div className="p-3 bg-blue-50 rounded">
                        <h5 className="font-medium text-blue-700">With VIX</h5>
                        <div className="text-sm">
                          <div>Accuracy: {evaluationResults.evaluation_summary.cross_validation_summary.with_vix?.accuracy?.mean?.toFixed(4)} (±{evaluationResults.evaluation_summary.cross_validation_summary.with_vix?.accuracy?.std?.toFixed(4)})</div>
                          <div>F1 Score: {evaluationResults.evaluation_summary.cross_validation_summary.with_vix?.f1_score?.mean?.toFixed(4)} (±{evaluationResults.evaluation_summary.cross_validation_summary.with_vix?.f1_score?.std?.toFixed(4)})</div>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {evaluationResults.evaluation_summary?.statistical_significance && (
                  <div className="mb-4 p-3 bg-gray-50 rounded">
                    <h4 className="font-semibold mb-2">Statistical Significance Test</h4>
                    <div className="text-sm">
                      <div>McNemar's Test P-value: {evaluationResults.evaluation_summary.statistical_significance.mcnemar_p_value?.toFixed(6)}</div>
                      <div className="mt-2 font-medium">
                        {evaluationResults.evaluation_summary.statistical_significance.interpretation}
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div className="mt-4 p-3 bg-green-50 rounded border border-green-200">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="font-medium text-green-900">Evaluation Complete</h4>
                      <p className="text-sm text-green-700">
                        Detailed results and visualizations saved to: {evaluationResults.output_directory}
                      </p>
                    </div>
                    <div className="text-xs text-green-600">
                      📁 Check evaluation_results/ folder for charts and full report
                    </div>
                  </div>
                </div>
              </div>
            ...
            {hmmPredictedRegimes && hmmChartData.length > 0 && (
                <div className="mt-4">
                    <h3 className="text-lg font-semibold mb-2">🔮 HMM Predicted Regimes (Next 2 Weeks)</h3>
                    ...
                    <ResponsiveContainer width="100%" height={200}>
                      <BarChart data={hmmChartData} margin={{ top: 5, right: 20, left: -20, bottom: 5 }}>
                        ...
                        <Tooltip formatter={(value: number, name: string, props: { payload: { date: string; regimeValue: number; regimeLabel: string } }) => [props.payload.regimeLabel, 'Regime']} />
      ...
      <div className="p-4 border rounded shadow-sm bg-white">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-semibold">🤖 ML Ensemble System (Random Forest + Gradient Boosting)</h2>
          {mlEnsembleStatus && (
            <div className="flex items-center space-x-2">
              ...
              <span className={`px-2 py-1 rounded text-xs ${
                ...
                {mlEnsembleStatus.status === 'trained' ? '🟢 Trained' : '🟡 Ready'}
              </span>
            </div>
          )}
        </div>
          ...
          <div className="mb-4 p-3 bg-gray-50 rounded">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              <div>
                ...
                <span className={`ml-2 ${mlEnsembleStatus.models_trained ? 'text-green-600' : 'text-red-600'}`}>
                  {mlEnsembleStatus.models_trained ? '✅ Yes' : '❌ No'}
                </span>
              </div>
              <div>
                <span className="font-medium">Training History:</span>
                <span className="ml-2">{mlEnsembleStatus.training_history_count || 0} sessions</span>
              </div>
                ...
                <>
                  <div>
                    ...
                    <span className={`ml-2 font-bold ${
                      mlEnsembleStatus.latest_performance.ensemble?.accuracy >= 0.65 
                        ? 'text-green-600' 
                        : 'text-red-600'
                    }`}>
                      {(mlEnsembleStatus.latest_performance.ensemble?.accuracy * 100).toFixed(1)}%
                    </span>
                  </div>
                  <div>
                    ...
                    <span className={`ml-2 ${
                      ...
                      {mlEnsembleStatus.meets_65_percent_requirement ? '✅ Met' : '❌ Not Met'}
                    </span>
                  </div>
                </>
              )}
        ...
        <div className="flex space-x-4 mb-4">
          ...
          <button
            ...
            disabled={mlLoading || mlTraining || !mlEnsembleStatus?.models_trained}
          ...
          <div className="mb-4 p-4 bg-white border rounded">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <div className="text-sm space-y-1">
                  <div>Random Forest: <span className="font-mono">{(mlTrainingResults.performance?.random_forest * 100).toFixed(1)}%</span></div>
                  <div>Gradient Boosting: <span className="font-mono">{(mlTrainingResults.performance?.gradient_boosting * 100).toFixed(1)}%</span></div>
                  <div className="font-medium">Ensemble: <span className={`font-mono ${
                    mlTrainingResults.performance?.ensemble >= 0.65 ? 'text-green-600' : 'text-red-600'
                  }`}>{(mlTrainingResults.performance?.ensemble * 100).toFixed(1)}%</span></div>
                </div>
              </div>
              <div className="space-y-2">
                <h4 className="font-medium text-gray-800">Training Info</h4>
                <div className="text-sm space-y-1">
                  <div>Training Samples: <span className="font-mono">{mlTrainingResults.training_samples}</span></div>
                  <div>Features: <span className="font-mono">{mlTrainingResults.features_count}</span></div>
                  <div>Model Saved: <span className="font-mono text-xs">{mlTrainingResults.model_path}</span></div>
                </div>
              </div>
            </div>
            ...
            {mlTrainingResults.meets_65_percent_requirement ? (
              <div className="mt-3 p-2 bg-green-50 border border-green-200 rounded">
                <p className="text-green-800 text-sm">✅ Model meets the 65% accuracy requirement!</p>
              </div>
            ) : (
              <div className="mt-3 p-2 bg-yellow-50 border border-yellow-200 rounded">
                <p className="text-yellow-800 text-sm">⚠️ Model accuracy below 65% target. Consider feature engineering or more data.</p>
              </div>
            )}
          </div>
        )}
        ...
        {mlPredictions && (
          <div className="mb-4 p-4 bg-white border rounded">
              ...
              <table className="min-w-full text-xs border">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="border px-2 py-1 text-center">Ensemble Confidence</th>
                    <th className="border px-2 py-1 text-center">Random Forest</th>
                    <th className="border px-2 py-1 text-center">Gradient Boosting</th>
                  </tr>
                </thead>
                <tbody>
                  {mlPredictions.predictions?.slice(0, 10).map((pred: any, idx: number) => (
                    <tr key={idx} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      <td className="border px-2 py-1 font-mono text-xs">{pred.date.slice(0, 10)}</td>
                      ...
                      <td className="border px-2 py-1 text-center font-mono">{(pred.ensemble_confidence * 100).toFixed(1)}%</td>
                      <td className="border px-2 py-1 text-center font-mono">{(pred.random_forest_confidence * 100).toFixed(1)}%</td>
                      <td className="border px-2 py-1 text-center font-mono">{(pred.gradient_boosting_confidence * 100).toFixed(1)}%</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="mt-2 text-xs text-gray-500">
              📝 Showing {Math.min(10, mlPredictions.predictions?.length || 0)} of {mlPredictions.predictions?.length || 0} predictions
            </div>
          </div>
        )}
      </div>
      ...
      {downloadedData && (
        <div className="p-4 border rounded shadow-sm bg-white">
          ...
          <div className="mb-3 text-sm text-gray-600">
            ...
            <span className="font-medium ml-2">File:</span> {downloadedData.filename} |
            <span className="font-medium ml-2">Showing:</span> First {downloadedData.data.length} rows
            {downloadedData.dateRange && (
              <>
                {' | '}
                <span className="font-medium">Range:</span> {downloadedData.dateRange.start} to {downloadedData.dateRange.end}
              </>
            )}
          </div>
          ...
          {downloadedData.data.length > 0 ? (
                ...
                <thead className="bg-gray-50">
                  <tr>
                    {Object.keys(downloadedData.data[0]).map((header) => (
                      <th key={header} className="border px-2 py-1 font-medium text-left">
                        {header}
                      </th>
                    ))}
                  </tr>
            ...
            <p className="text-gray-500">No data rows found in the downloaded file.</p>
          )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {summaryMetrics && !loading && !error && (
          <div className="p-4 border rounded shadow-sm bg-white">
            ...
            <ul className="list-disc pl-5">
              {Object.entries(summaryMetrics.pnlByStrategy).map(([strategy, data]) => (
                <li key={strategy}>{strategy}: ${data.totalPL.toFixed(2)} ({data.count} trades)</li>
              ))}
            </ul>
          </div>
        )}
        ...
        {!loading && !error && (
          <div className="p-4 border rounded shadow-sm bg-white">
            ...
            <div className={`p-3 rounded text-center border ${getRegimeClasses(currentMarketRegime)}`}>
            ...
            {regimeHistory.length > 0 && (
             ...
             {regimeHistory.length === 0 && currentMarketRegime === VolatilityRegime.UNKNOWN && (
              <p className="text-gray-500 mt-3 text-sm">Not enough historical trade data to determine regime or history.</p>
            )}
          </div>
        )}
      </div>
      ...
      <div className="p-4 border rounded shadow-sm bg-white mt-6">
        ...
        {!loading && !error && trades.length === 0 && (
           <p className="text-gray-500">No trade data available to display visualizations.</p>
        )}
        {/* Export Button */}
      ...
      <div className="p-4 border rounded shadow-sm bg-white mt-6">
        ...
        <div className="flex items-center gap-4 mb-2 flex-wrap">
          ...
          <button
            className="border px-2 py-1 rounded text-xs"
            onClick={() => setInsightsSort(s => s === 'asc' ? 'desc' : 'asc')}
        ...
        {featuresLoading && <div className="py-4 text-center"><span className="animate-spin inline-block w-6 h-6 border-4 border-blue-300 border-t-transparent rounded-full" aria-label="Loading features"></span></div>}
        {featuresError && <div className="py-2 text-red-600 text-center">{featuresError}</div>}
        ...
        {!featuresLoading && !featuresError && featureImportances.length === 0 && (
          <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 my-4 rounded">
            <div className="font-semibold text-yellow-800 mb-1">No feature importances or explainability data found.</div>
            <div className="text-yellow-700 text-sm mb-2">
              To see advanced analytics (feature importance, SHAP values), please train a model and export results.<br />
              <span className="font-medium">Run this command in your project root:</span>
            </div>
            <pre className="bg-gray-100 rounded px-2 py-1 text-xs text-gray-800 mb-2">npx tsx scripts/train_baseline_model.ts</pre>
            <div className="text-xs text-gray-600">After running, refresh this page to see updated analytics.</div>
          </div>
        )}
        {/* Model predictions per trade (e.g., win streak, regime) */}
        {!featuresLoading && !featuresError && filteredFeatures.length > 0 && (
          <div className="mt-4 overflow-x-auto">
            <h3 className="text-md font-semibold mb-2">Model Predictions (Win Streak, Regime)</h3>
            <table className="min-w-full text-xs border">
              <thead>
                <tr>
                  <th className="border px-2 py-1" title="Trade ID">Trade ID</th>
                  <th className="border px-2 py-1" title="Symbol">Symbol</th>
                  <th className="border px-2 py-1" title="Consecutive win/loss streak">Streak <span title="Consecutive win/loss streak" aria-label="Consecutive win/loss streak">🛈</span></th>
                  <th className="border px-2 py-1" title="Market regime classification">Regime <span title="Market regime classification" aria-label="Market regime classification">🛈</span></th>
                </tr>
              </thead>
              <tbody>
                {filteredFeatures.slice(0, 20).map((f, i) => (
                  <tr key={f.id || i} className={f.streak >= 2 ? 'bg-green-50' : ''} onClick={() => setSelectedTradeIdx(i)} style={{ cursor: 'pointer' }} title="Click for details">
            ...
            <TradeDetailModal
              ...
              features={selectedTradeIdx !== null ? features[selectedTradeIdx] : null}
            />
          </div>
        )}
      </div>
      ...
      <div className="p-4 border rounded shadow-sm bg-white mt-6">
        ...
        <p className="mb-2 text-gray-600">Trades currently in a win streak (streak ≥ 2):</p>
          ...
          <table className="min-w-full text-xs border">
            ...
            <tbody>
              {winZoneTrades.map((f, i) => (
                <tr key={f.id || i} onClick={() => setSelectedTradeIdx(i)} style={{ cursor: 'pointer' }} title="Click for details">
                  <td className="border px-2 py-1">{f.id}</td>
                  <td className="border px-2 py-1">{trades[i]?.symbol}</td>
                  <td className="border px-2 py-1">{f.streak}</td>
      ...
      <div className="p-4 border rounded shadow-sm bg-white mt-6">
        ...
        <p className="mb-2 text-gray-600">Timeline of win/loss streaks and market regimes.</p>
        {features.length > 0 ? (
          <InsightsResponsiveContainer width="100%" height={250}>
            <InsightsBarChart data={features.map((f, i) => ({
              date: trades[i]?.closeDate?.toISOString().split('T')[0] || trades[i]?.openDate?.toISOString().split('T')[0] || i,
              streak: f.streak,
              regime: f.marketRegime,
            }))}>
              <InsightsCartesianGrid strokeDasharray="3 3" />
              <InsightsXAxis dataKey="date" tick={{ fontSize: 10 }} />
              <InsightsYAxis yAxisId="left" orientation="left" label={{ value: 'Streak', angle: -90, position: 'insideLeft' }} />
              <InsightsYAxis yAxisId="right" orientation="right" label={{ value: 'Regime', angle: 90, position: 'insideRight' }} />
              <InsightsTooltip />
              <InsightsBar yAxisId="left" dataKey="streak" fill="#10B981" name="Streak" />
              {/* Regime as a bar with color coding */}
              <InsightsBar yAxisId="right" dataKey="regime" fill="#6366F1" name="Regime" />
            </InsightsBarChart>
          </InsightsResponsiveContainer>
        ) : (
      ...
      <div className="p-4 border rounded shadow-sm bg-white mt-6">
        ...
        {showHowItWorks && (
          <div className="mt-2 text-sm text-gray-700">
            <h3 className="font-semibold mb-1">About the HMM Model & Regime Analysis</h3>
            <ul className="list-disc pl-5 mb-2">
              <li>The Hidden Markov Model (HMM) is trained on your trade and market data to identify market regimes (e.g., HighVol, MediumVol, LowVol).</li>
              <li>Regime predictions help you understand periods of high or low volatility, which can inform your trading strategy.</li>
              <li>All calculations are based on your imported trades and the selected market data range.</li>
              <li>For more details, see the project documentation or ask for help in the Troubleshooting section above.</li>
            </ul>
          </div>
        )}
      </div>
