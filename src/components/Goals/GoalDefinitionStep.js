...
import GoalFlowIntegration from '../../services/userFlow/GoalFlowIntegration';

const GoalDefinitionStep = ({ 
  userProfile, 
  onStepComplete, 
  onProgress,
  className = '',
  ...
  const [assessmentStatus, setAssessmentStatus] = useState({ status: 'not_started' });
  ...
  useEffect(() => {
    ...
    const handleEducationalIntervention = (data) => {
      if (data.interventionType === 'bias_education') {
        setEducationalContent(getBiasEducationContent(data.biases));
        setShowEducation(true);
      } else if (data.interventionType === 'conflict_resolution') {
  ...
  const handleQuestionnaireComplete = useCallback(async (data) => {
    try {
      setIsLoading(true);
  ...
  const handleGoalRefinement = useCallback(async () => {
    try {
  ...
  const getBiasEducationContent = (biases) => {
    const biasContent = {
      overconfidence: {
  ...
  const getConflictResolutionContent = (conflicts) => {
    return {
    ...
    <div className={`goal-definition-step ${className}`}>
      ...
      {assessmentStatus.status !== 'not_started' && !showResults && (
        <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          {assessmentStatus.detectedBiases?.length > 0 && (
          ...
          {assessmentStatus.goalCandidates?.length > 0 && (
