 ...
 * Main Demo Component
 */
const TemplateMatchingDemo = () => {
  const [demoMode, setDemoMode] = useState('overview'); // overview, live_demo, research_metrics
  ...
  const [demoAccountInfo, setDemoAccountInfo] = useState({
    size: 50000,
  ...
  const handleStockSelect = (selectedStocks) => {
    console.log('Selected stocks:', selectedStocks);
  };

  const handleEducationRequest = (request) => {
    ...
    <div className="max-w-7xl mx-auto p-6 space-y-8">
      ...
      <div className="text-center">
        ...
        <div className="flex justify-center gap-2">
          {[
            { id: 'overview', label: 'System Overview', icon: LightBulbIcon },
            { id: 'live_demo', label: 'Live Demo', icon: PlayIcon },
            { id: 'research_metrics', label: 'Research Metrics', icon: TrophyIcon }
 ...
 */
const SystemOverview = ({ onStartDemo }) => {
  const features = [
    {
      icon: CpuChipIcon,
