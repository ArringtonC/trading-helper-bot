...
import { UserFlowManager } from '../../services/userFlow/UserFlowManager';

const GoalAssessmentContainer = ({ 
  userProfile,
  ...
  const handleQuestionnaireProgress = (progressData) => {
  ...
  const handleQuestionnaireComplete = async (completeData) => {
    setAssessmentData(prev => ({
      ...prev,
    ...
    if (completeData.assessment?.conflicts?.length > 0) {
  ...
  const handleConflictResolved = async (conflict, resolution) => {
    setAssessmentData(prev => ({
      ...prev,
  ...
  const handleValidationComplete = async (validationResults) => {
    setAssessmentData(prev => ({
      ...prev,
      validationResults
    }));
  ...
  const updateGoalsBasedOnResolution = async (goals, conflict, resolution) => {
  ...
  const getStepProgress = () => {
    const steps = ['questionnaire', 'conflict_resolution', 'realism_validation', 'education', 'summary'];
 ...
 */
const AssessmentSummary = ({ assessmentData, onComplete }) => {
  return (
          ...
          <div key={index} className="bg-white border border-gray-200 rounded-lg p-6">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium">{Math.round(goal.score * 100)}%</span>
              </div>
              <div className="flex justify-between">
