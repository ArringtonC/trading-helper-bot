...
import AccountLevelSystem, { ACCOUNT_LEVELS, REGULATORY_THRESHOLDS } from '../../services/account/AccountLevelSystem';
...
const AccountClassificationInterface = () => {
  const [accountData, setAccountData] = useState(getDefaultAccountData());
  const [classificationResult, setClassificationResult] = useState(null);
  const [loading, setLoading] = useState(false);
  const [errors, setErrors] = useState({});
  const [accountLevelSystem, setAccountLevelSystem] = useState(null);
  const [growthMonitoring, setGrowthMonitoring] = useState(null);
   ...
   * Classify account
   */
  const classifyAccount = useCallback(async () => {
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                {growthMonitoring.riskPatterns?.length > 0 ? (
                  <div className="space-y-1">
