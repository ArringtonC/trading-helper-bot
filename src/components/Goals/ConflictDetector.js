...
const ConflictDetector = ({ 
  ...
  const getConflictDisplayInfo = (conflict) => {
    const conflictMap = {
  ...
  const getEducationalContent = (conflict) => {
    const educational = {
      safety_profit_conflict: {
        riskReturnEducation: {
  ...
  const handleStartResolution = (conflict) => {
    setShowResolution(conflict);
  };

  const handleResolutionStep = (conflictId, step, decision) => {
  ...
  const handleCompleteResolution = (conflict, resolution) => {
  ...
  return (
    <div className={`conflict-detector ${className}`}>
      ...
      <ProgressiveDisclosure
        isOpen={showResolution !== null}
        onClose={() => setShowResolution(null)}
 ...
 */
const ConflictResolutionWizard = ({ conflict, responses, onComplete, onCancel }) => {
  ...
  const getResolutionSteps = (conflict, responses) => {
    const baseSteps = {
      safety_profit_conflict: [
        {
...
const PriorityClarificationStep = ({ onComplete }) => (
  <div className="space-y-4">
...
const EducationStep = ({ educationalContent, onComplete }) => (
  <div className="space-y-4">
...
const StrategySelectionStep = ({ onComplete }) => (
  <div className="space-y-4">
...
const TimelineReviewStep = ({ onComplete }) => <div>Timeline Review - To be implemented</div>;
const StrategyAlignmentStep = ({ onComplete }) => <div>Strategy Alignment - To be implemented</div>;
const GenericResolutionStep = ({ onComplete }) => (
  <div>
    <p>Generic resolution step</p>
