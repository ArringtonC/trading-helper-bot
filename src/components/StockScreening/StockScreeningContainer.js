 ...
 */
export const StockScreeningContainer = ({
  initialStocks = [],
  onStockSelection,
  onPortfolioUpdate,
  className = ''
  ...
  const [sortBy, setSortBy] = useState('goalMatch');
  ...
  const alignmentStats = useMemo(() => {
    ...
    const highAlignmentCount = stocksWithAlignment.filter(s => 
      s.goalAlignment.score >= RESEARCH_CONSTANTS.GOAL_ALIGNMENT_THRESHOLD).length;
      ...
      <div className="screening-header">
        <div className="header-content">
          <h1 className="screening-title">
            {goals 
              ? `Stocks for ${goals.primary?.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase())} Goals`
              : 'Stock Screening'
            }
          </h1>
