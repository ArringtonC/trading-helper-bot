 ...
 */
const GoalIdentificationWizard = ({
  onGoalIdentified,
  ...
  const performAnalysis = async (currentResponses) => {
    setIsAnalyzing(true);
    try {
  ...
  const isStepComplete = (step, currentResponses) => {
  ...
  const handleNext = () => {
    if (currentStep < questionnaire.sections.length - 1) {
        ...
        return (
          <div className="space-y-4">
            <div className="flex justify-between text-sm text-gray-600">
              <span>{question.labels?.[question.scale.min]}</span>
              <span>{question.labels?.[question.scale.max]}</span>
            </div>
            <input
              type="range"
          ...
          {analysis.biasesDetected.map((bias, index) => (
            <div key={index} className="p-4 bg-white rounded-lg border border-orange-200">
              <div className="font-medium text-orange-800 mb-2">
                {goalSystem.psychologicalBiases[bias.type]?.name}
              </div>
              <div className="text-sm text-orange-700 mb-2">
                {goalSystem.psychologicalBiases[bias.type]?.description}
              </div>
