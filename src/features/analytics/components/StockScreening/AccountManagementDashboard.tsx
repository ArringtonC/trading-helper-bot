...
interface AccountLevel {
  ...
  upgradeThresholds?: UpgradeThreshold;
  adjustedMetrics?: {
...
interface AccountManagementDashboardProps {
  ...
  onAccountUpdate?: (level: AccountLevel) => void;
...
const AccountManagementDashboard: React.FC<AccountManagementDashboardProps> = ({
  ...
  const [positionCalculations, setPositionCalculations] = useState<Record<string, PositionCalculation>>({});
  ...
  const calculatePositionSize = (stockData: StockData): PositionCalculation | null => {
  ...
  const handleStockSelection = (stockData: StockData): void => {
    setSelectedStock(stockData);
    calculatePositionSize(stockData);
  };

  const renderAccountTier = (): JSX.Element | null => {
    if (!accountLevel || !accountData) return null;
                ...
                <div className="space-y-1">
                  {tier.restrictions.map((restriction, index) => (
                    <div key={index} className="text-sm text-orange-600">
                      • {restriction.replace(/-/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())}
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="w-full bg-blue-200 rounded-full h-2">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, ((accountData.accountBalance || 0) / upgradeThresholds.balanceRequired) * 100)}%`
                  }}
                />
              </div>
              
              <div className="text-xs text-blue-600">
                ${(upgradeThresholds.balanceRequired - (accountData.accountBalance || 0)).toLocaleString()} 
  ...
  const renderComplianceWarnings = (): JSX.Element | null => {
  ...
  const renderPositionSizingCalculator = (): JSX.Element | null => {
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
        ...
        <div className="space-y-4">
          ...
          {selectedStock?.stockPrice && (
            <button
              onClick={() => selectedStock && calculatePositionSize(selectedStock)}
  ...
  const renderPositionSizingResults = (calculation: PositionCalculation): JSX.Element | null => {
