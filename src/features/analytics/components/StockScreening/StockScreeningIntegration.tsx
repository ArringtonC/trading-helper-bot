...
import UserFlowContext, { useUserFlow } from '../../../../shared/context/UserFlowContext';
// import { useGoalFirstFlow } from '../../../../shared/hooks/useGoalFirstFlow';
// import GoalBasedTemplateSystem from '../../../../shared/services/matching/GoalBasedTemplateSystem';
...
interface StockMatch {
  stock: any;
  alignmentScore: { totalScore: number };
  recommendations: any[];
  riskAssessment: any;
  expectedPerformance: any;
}

interface StockMatches {
  matches: StockMatch[];
  summary: any;
  template: any;
  aiInsights: any;
...
interface StockScreeningIntegrationProps {
  initialUserData?: UserData | null;
  initialAccountData?: AccountData | null;
  onScreeningComplete?: (data: CompletionData) => void;
  className?: string;
}

interface FlowHookFallback {
  currentStep: string;
  progress: number;
  stepData: any;
...
const StockScreeningIntegrationWrapper: React.FC<StockScreeningIntegrationProps> = (props) => {
  try {
    return <StockScreeningIntegration {...props} />;
  } catch (error) {
    if (error instanceof Error && error.message.includes('useUserFlow must be used within a UserFlowProvider')) {
...
const StockScreeningIntegration: React.FC<StockScreeningIntegrationProps> = ({
  ...
  // State management
  const [userGoals, setUserGoals] = useState<UserData | null>(null);
  const [accountLevel, setAccountLevel] = useState<AccountData | null>(null);
  const [customTemplate, setCustomTemplate] = useState<any>(null);
  const [stockMatches, setStockMatches] = useState<StockMatches | null>(null);
   ...
   */
  const handleGoalIdentification = async (goalData: GoalData): Promise<void> => {
   ...
   */
  const handleAccountUpdate = async (accountData: AccountData): Promise<void> => {
    setIsProcessing(true);
    try {
   ...
   */
  const handleScreeningResults = async (results: ScreeningResults): Promise<void> => {
    setIsProcessing(true);
    try {
      // Use template-based matching if available
      let matches: StockMatches;
   ...
   */
  const handleStockSelection = (stocks: any[]): void => {
