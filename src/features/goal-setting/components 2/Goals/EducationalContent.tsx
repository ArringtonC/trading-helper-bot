...
interface UnrealisticExpectation {
  type: string;
  description: string;
...
interface EducationalContentProps {
  ...
  detectedBiases?: Bias[];
  unrealisticExpectations?: UnrealisticExpectation[];
  className?: string;
}
...
interface ModuleContent {
  ...
  strategies?: Strategy[];
  historicalReturns?: Record<string, HistoricalReturn>;
  realityCheck?: string[];
  compoundingExamples?: CompoundingExample[];
  fundamentalCriteria?: string[];
  qualitativeFactor?: string[];
  smallCapGrowth?: SmallCapGrowth;
  defensiveCriteria?: string[];
  inflationProtection?: string[];
  graharmCriteria?: GrahamCriteria;
  benefits?: string[];
...
interface EducationalModuleComponentProps {
...
const EducationalContent: React.FC<EducationalContentProps> = ({ 
  ...
  const getEducationalModules = (goalType: GoalType): EducationalModule[] => {
    const moduleMap: Record<GoalType, EducationalModule[]> = {
      income_generation: [
        {
          id: 'dividend_basics',
  ...
  const getBiasEducation = (biases: Bias[]): BiasEducation[] => {
    const biasEducation: Record<BiasType, BiasEducation> = {
      overconfidence: {
      ...
      projection: {
        title: 'Managing Projection Bias',
        description: 'Tendency to project current emotions onto future market conditions.',
        strategies: [
          'Create written investment rules and stick to them',
          'Use systematic entry and exit strategies',
          'Regular portfolio reviews independent of market emotions',
          'Understand that current market conditions are temporary'
        ],
        behavioralInsights: [
          'Projection bias leads to poor timing decisions',
          'Can result in inconsistent investment strategies',
          'Often causes emotional rather than rational decision-making'
        ]
      }
    };
 ...
 */
const EducationalModule: React.FC<EducationalModuleComponentProps> = ({ module, isExpanded, onToggle }) => {
 ...
 */
const ModuleContent: React.FC<ModuleContentComponentProps> = ({ content }) => {
  return (
