...
type RecommendationStrength = 'weak' | 'moderate' | 'strong';
...
interface StockRecommendationCardProps {
  ...
  onEducationClick?: (warnings: Warning[]) => void;
  isSelected?: boolean;
...
const StockRecommendationCard: React.FC<StockRecommendationCardProps> = ({ 
  stock, 
  onSelect, 
  onViewDetails, 
  onEducationClick,
 ...
 */
const WarningModal: React.FC<WarningModalProps> = ({ stock, onClose, onEducationClick }) => {
  ...
  const getWarningIcon = (severity: SeverityLevel): ReactNode => {
    switch (severity) {
  ...
  const getWarningColor = (severity: SeverityLevel): string => {
    switch (severity) {
 ...
 */
const WarningCard: React.FC<WarningCardProps> = ({ warning, onEducationClick }) => {
  const getWarningIcon = (severity: SeverityLevel): ReactNode => {
    switch (severity) {
      case 'high': return <ExclamationTriangleIcon className="w-5 h-5 text-red-500" />;
      case 'medium': return <ExclamationTriangleIcon className="w-5 h-5 text-yellow-500" />;
