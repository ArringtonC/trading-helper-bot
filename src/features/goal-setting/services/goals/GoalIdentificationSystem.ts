...
type RiskLevel = 'none' | 'low' | 'medium' | 'medium-high' | 'high' | 'very-high';
type GoalId = 'income' | 'growth' | 'preservation' | 'learning' | 'active';
type ExperienceLevel = 'none' | 'beginner' | 'intermediate' | 'advanced';
type TimeHorizon = 'short' | 'medium' | 'long';
type QuestionType = 'single-select' | 'multi-select' | 'scale' | 'number' | 'boolean';
type BiasType = 'PROJECTION_BIAS' | 'OVERCONFIDENCE_BIAS' | 'LOSS_AVERSION';
type ConflictType = 'SAFETY_QUICK_PROFITS' | 'UNREALISTIC_RETURNS';
type RecommendationType = 'strategy' | 'education' | 'warning';
type RecommendationPriority = 'low' | 'medium' | 'high' | 'critical';
type RiskCategory = 'conservative' | 'moderate' | 'aggressive' | 'very_aggressive';
type ValidationType = 'regulatory' | 'experience_mismatch';
...
interface PayoutRatio {
  max: number;
}

interface EarningsConsistency {
  years: number;
}

interface GoalCharacteristics {
  ...
  epsGrowth?: EpsGrowth;
  pegRatio?: PegRatio;
  smallCapAdvantage?: number;
  earningsConsistency?: EarningsConsistency;
  marketPosition?: string;
  paperTrading?: boolean;
  mistakeFriendly?: boolean;
  educationalValue?: string;
  minEquity?: number;
  volatility?: string;
  warningLevel?: string;
  dayTradingRisk?: string;
}
...
interface GoalConflictDefinition {
  conflict: string;
  resolution: string;
  educationRequired: boolean;
  thresholds?: {
    monthly?: { unrealistic: number };
    annual?: { aggressive: number; unrealistic: number };
  };
}
...
interface GoalAnalysis {
  primaryGoal: GoalCategory | null;
  secondaryGoals: GoalCategory[];
  riskProfile: RiskProfile | null;
  biasesDetected: BiasDetection[];
  conflicts: GoalConflict[];
  recommendations: Recommendation[];
  educationNeeded: EducationNeed[];
}
...
interface QuestionnaireResponses {
  ...
  margin_usage?: boolean;
  [key: string]: any;
}

class GoalIdentificationSystem {
  public goalCategories: Record<string, GoalCategory>;
  public psychologicalBiases: Record<BiasType, PsychologicalBias>;
  public goalConflicts: Record<ConflictType, GoalConflictDefinition>;
   ...
   */
  createGoalQuestionnaire(userLevel: UserLevel = 'beginner'): Questionnaire {
   ...
   */
  getProgressiveQuestions(userLevel: UserLevel): Question[] {
    const allQuestions: Record<UserLevel, Question[]> = {
      beginner: [
        {
   ...
   */
  identifyTraderGoals(responses: QuestionnaireResponses): GoalAnalysis {
    ...
    const goalKey = (responses.primary_goal?.toUpperCase() + '_' + 
      (responses.primary_goal === 'income' ? 'GENERATION' : 
       ...
       responses.primary_goal === 'active' ? 'TRADING' : '')) as keyof typeof this.goalCategories;
   ...
   */
  assessRiskProfile(responses: QuestionnaireResponses): RiskProfile {
    const riskScore = responses.risk_tolerance || 5;
    const timeHorizon = responses.time_horizon || 'medium';
    const experience = responses.experience_level || 'beginner';
   ...
   */
  detectBiases(responses: QuestionnaireResponses): BiasDetection[] {
    const biases: BiasDetection[] = [];

    // Check for unrealistic return expectations
    if ((responses.monthly_income_expectation || 0) > 15) {
      biases.push({
    ...
    if ((responses.risk_tolerance || 0) > 8 && responses.loss_comfort === 'panic') {
      biases.push({
    ...
    if (responses.loss_comfort === 'panic' && (responses.risk_tolerance || 0) < 4) {
      biases.push({
        type: 'LOSS_AVERSION',
   ...
   */
  identifyGoalConflicts(responses: QuestionnaireResponses, primaryGoal: GoalCategory | null): GoalConflict[] {
    const conflicts: GoalConflict[] = [];

    // Check for safety + quick profits conflict
    if (primaryGoal?.id === 'preservation' && (responses.monthly_income_expectation || 0) > 10) {
      conflicts.push({
    ...
    // Check for unrealistic return expectations
    if ((responses.monthly_income_expectation || 0) > 20) {
      conflicts.push({
   ...
   */
  generateRecommendations(responses: QuestionnaireResponses, analysis: GoalAnalysis): Recommendation[] {
    ...
    if (analysis.riskProfile && analysis.riskProfile.score < 4) {
      recommendations.push({
        type: 'education',
   ...
   */
  determineEducationNeeds(analysis: GoalAnalysis): EducationNeed[] {
    const education: EducationNeed[] = [];
   ...
   */
  validateGoalRealism(goals: GoalAnalysis, accountSize: number, experience: ExperienceLevel): GoalValidation {
    ...
    if (experience === 'none' && goals.primaryGoal?.characteristics.riskLevel === 'very-high') {
      validation.warnings.push({
   ...
   */
  getGoalCategory(goalId: GoalId): GoalCategory | undefined {
   ...
   */
  getAllGoalCategories(): GoalCategory[] {
