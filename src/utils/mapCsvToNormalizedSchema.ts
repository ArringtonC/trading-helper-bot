...
const formatDate = (dateStr?: string): string => {
  ...
  try {
    const date = new Date(dateStr.replace(/(\d{4})(\d{2})(\d{2})/, '$1-$2-$3')); // Handle YYYYMMDD
...
export const mapRowToNormalizedTradeData = (
  ...
  if (broker === BrokerType.IBKR) {
    ...
    data.tradePrice = parseSafeNumber(get(['TradePrice', 'Trade Price'])) ?? 0;
    ...
    data.netAmount = parseSafeNumber(get(['NetCash', 'TradeMoney'])) ?? 0; // TradeMoney can be negative for debit
    ...
    data.multiplier = parseSafeNumber(get(['Multiplier'])) ?? (data.assetCategory === 'OPT' ? 100 : 1);
