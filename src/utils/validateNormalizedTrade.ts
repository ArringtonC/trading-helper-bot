import { NormalizedTradeData, AssetCategory, BrokerType, OpenCloseIndicator, PutCall } from '../types/trade';

const DATE_REGEX = /^\d{4}-\d{2}-\d{2}$/;

export const validateNormalizedTradeData = (trade: NormalizedTradeData | null): string[] => {
  const errors: string[] = [];
  ...
  if (!trade.tradeDate || typeof trade.tradeDate !== 'string') {
    ...
    errors.push(`Invalid tradeDate format: ${trade.tradeDate}. Expected YYYY-MM-DD.`);
  }
  ...
  const validAssetCategories: AssetCategory[] = ['STK', 'OPT', 'FUT', 'CASH', 'FX', 'BOND', 'Unknown'];
  ...
  if (trade.settleDate && (!DATE_REGEX.test(trade.settleDate) || isNaN(new Date(trade.settleDate).getTime()))) {
    errors.push(`Invalid settleDate format: ${trade.settleDate}. Expected YYYY-MM-DD.`);
  }
  ...
  const validOpenCloseIndicators: OpenCloseIndicator[] = ['O', 'C', 'N/A'];
  ...
  if (trade.assetCategory === 'OPT') { // Compare with string literal 'OPT'
    if (!trade.expiryDate || !DATE_REGEX.test(trade.expiryDate) || isNaN(new Date(trade.expiryDate).getTime())) {
      errors.push(`Options require a valid expiryDate (YYYY-MM-DD): ${trade.expiryDate}.`);
    }
    ...
    const validPutCallIndicators: PutCall[] = ['P', 'C', 'N/A'];
  ...
  if (trade.tradePrice !== undefined && trade.quantity !== undefined && trade.netAmount !== undefined) {
    const expectedNet = (trade.tradePrice * trade.quantity * (trade.multiplier || 1));
    let calculatedNet = expectedNet;
    if (trade.commission) calculatedNet -= Math.abs(trade.commission);
    if (trade.fees) calculatedNet -= Math.abs(trade.fees);
