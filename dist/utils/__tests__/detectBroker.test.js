...
describe('detectBroker', function () {
    it('should detect IBKR from characteristic headers', function () {
        var ibkrHeaders = [
    ...
    it('should detect Schwab from characteristic headers', function () {
        var schwabHeaders = [
    ...
    it('should return Unknown for empty headers', function () {
        expect(detectBroker([])).toBe(BrokerType.Unknown);
    });
    it('should return Unknown for generic headers', function () {
        var genericHeaders = ['Column A', 'Column B', 'Date', 'Value', 'Amount'];
        expect(detectBroker(genericHeaders)).toBe(BrokerType.Unknown);
    });
    it('should correctly identify minimal IBKR headers', function () {
        var minimalIbkr = ['Asset Category', 'Symbol', 'Currency', 'TradePrice'];
        expect(detectBroker(minimalIbkr)).toBe(BrokerType.IBKR);
    });
    it('should correctly identify minimal Schwab headers', function () {
        var minimalSchwab = ['Total proceeds', 'Total cost basis', 'Date acquired', 'Symbol'];
        expect(detectBroker(minimalSchwab)).toBe(BrokerType.Schwab);
    });
    it('should handle mixed headers and prefer stronger match if logic allows or default to Unknown', function () {
        // This test depends on the specific scoring and tie-breaking logic
        var mixedHeaders = ['Asset Category', 'Symbol', 'Total proceeds', 'Total cost basis', 'Some Other Column'];
        ...
        var result = detectBroker(mixedHeaders);
    ...
    it('should handle headers with different casing and whitespace', function () {
        var ibkrHeadersSpaced = [
            ' asset category ', ' ibcommission  ', 'tradeprice', 'SYMBOL', 'currency'
        ];
        expect(detectBroker(ibkrHeadersSpaced)).toBe(BrokerType.IBKR);
    });
    it('should return Unknown if not enough required headers are present for IBKR', function () {
        var notEnoughIbkr = ['IBCommission', 'TradePrice', 'TradeDate']; // Missing 'Asset Category' or 'Symbol' or 'Currency' (needs 2 of these)
        expect(detectBroker(notEnoughIbkr)).toBe(BrokerType.Unknown);
    });
    it('should return Unknown if not enough required headers are present for Schwab', function () {
        var notEnoughSchwab = ['Date acquired', 'Date sold', 'Quantity']; // Missing 'Total proceeds' or 'Total cost basis'
        expect(detectBroker(notEnoughSchwab)).toBe(BrokerType.Unknown);
    });
    // Example of a more specific IBKR report (Trades section)
    var ibkrTradeReportHeaders = [
