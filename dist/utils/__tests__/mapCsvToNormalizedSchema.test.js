...
import { BrokerType } from '../../types/trade'; // Adjust path as needed
describe('mapRowToNormalizedTradeData', function () {
    ...
    it('should correctly map an IBKR trade row to NormalizedTradeData', function () {
        var normalized = mapRowToNormalizedTradeData(ibkrRow, ibkrHeaders, BrokerType.IBKR);
    ...
    it('should correctly map a Schwab stock buy row to NormalizedTradeData', function () {
        var normalized = mapRowToNormalizedTradeData(schwabBuyRow, schwabHeaders, BrokerType.Schwab);
        expect(normalized).not.toBeNull();
        if (!normalized)
            return;
    ...
    it('should correctly map a Schwab option sell to close row', function () {
        var normalized = mapRowToNormalizedTradeData(schwabSellOptionRow, schwabHeaders, BrokerType.Schwab);
        expect(normalized).not.toBeNull();
        if (!normalized)
            return;
        expect(normalized.broker).toBe(BrokerType.Schwab);
    ...
    it('should return null if essential fields are missing for IBKR', function () {
        var incompleteIbkrRow = __assign(__assign({}, ibkrRow), { Symbol: undefined }); // Missing symbol
        var normalized = mapRowToNormalizedTradeData(incompleteIbkrRow, ibkrHeaders, BrokerType.IBKR);
        expect(normalized).toBeNull();
    });
    it('should return null if essential fields are missing for Schwab', function () {
        var incompleteSchwabRow = __assign(__assign({}, schwabBuyRow), { Date: undefined }); // Missing date
        var normalized = mapRowToNormalizedTradeData(incompleteSchwabRow, schwabHeaders, BrokerType.Schwab);
        expect(normalized).toBeNull();
    });
    // Add more tests for:
