 ...
 */
var OptionsSummary = function (_a) {
    var stats = _a.stats, accountId = _a.accountId;
    useEffect(function () {
    ...
    return (<div className="bg-white p-4 rounded-lg shadow border border-gray-200">
      <div className="flex justify-between items-center mb-4">
        ...
        <Link to="/options" className="text-blue-600 hover:text-blue-800 text-sm font-medium">
          View All →
        </Link>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="flex justify-between text-sm border-t border-gray-200 pt-2 mt-2">
          ...
          <span className={"font-bold ".concat(stats.totalPL >= 0 ? 'text-green-600' : 'text-red-600')}>
            ${stats.totalPL.toFixed(2)}
          </span>
        </div>
      </div>
      
      {stats.openTrades > 0 && (<div className="mt-4 pt-3 border-t border-gray-200">
          <h3 className="text-sm font-medium text-gray-600 mb-2">Quick Actions</h3>
          <div className="flex gap-2">
            <Link to={"/options/new?accountId=".concat(accountId)} className="text-xs bg-blue-600 hover:bg-blue-700 text-white px-2 py-1 rounded">
              New Trade
            </Link>
            <Link to="/options" className="text-xs bg-gray-600 hover:bg-gray-700 text-white px-2 py-1 rounded">
              Manage Positions
            </Link>
          </div>
        </div>)}
      
      {stats.totalTrades === 0 && (<div className="mt-4 pt-3 border-t border-gray-200 text-center">
          <p className="text-sm text-gray-500 mb-3">No options trades yet</p>
          <Link to={"/options/new?accountId=".concat(accountId)} className="inline-block text-xs bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded">
