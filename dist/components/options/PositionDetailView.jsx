 ...
 */
var PositionDetailView = function (_a) {
    var trade = _a.trade, onClose = _a.onClose, onClosePosition = _a.onClosePosition;
    // Calculate days until expiration
    var daysUntilExpiry = function () {
        var today = new Date();
    ...
    var calculateEstimatedPL = function () {
        ...
        var closeValue = (trade.closePremium || 0) * Math.abs(trade.quantity) * multiplier;
    ...
    var pl = calculateEstimatedPL();
    var days = daysUntilExpiry();
    var isExpired = days === 0 && !trade.closeDate;
    return (<div className="bg-white p-6 rounded-lg shadow border border-gray-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div>
          ...
          <div className="mb-4">
            ...
            <div className={"inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ".concat(trade.putCall === 'CALL' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800')}>
              {trade.putCall}
            </div>
          </div>
          ...
          <div className="mb-4">
            ...
            <div className="text-lg font-medium">
              ...
              {!trade.closeDate && (<span className={"ml-2 text-xs ".concat(isExpired ? 'text-red-600' : days <= 7 ? 'text-yellow-600' : 'text-green-600')}>
                  {isExpired ? 'Expired' : "".concat(days, " days remaining")}
                </span>)}
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          {trade.closeDate && (<div className="mb-4">
          ...
          {trade.closePremium !== undefined && (<div className="mb-4">
          ...
          {pl !== undefined && (<div className="mb-4">
              <div className="text-sm text-gray-500 mb-1">Profit/Loss</div>
              <div className={"text-lg font-medium ".concat(pl >= 0 ? 'text-green-600' : 'text-red-600')}>
      ...
      {trade.strategy && (<div className="mb-4">
          <div className="text-sm text-gray-500 mb-1">Strategy</div>
          <div className="text-base">{trade.strategy}</div>
        </div>)}
      
      {trade.notes && (<div className="mb-6">
          <div className="text-sm text-gray-500 mb-1">Notes</div>
          <div className="text-base bg-gray-50 p-3 rounded">{trade.notes}</div>
        </div>)}
      
      {!trade.closeDate && onClosePosition && (<div className="flex justify-end border-t border-gray-200 pt-4">
          <button onClick={function () { return onClosePosition(trade.id); }} className="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">
            Close Position
          </button>
        </div>)}
    </div>);
