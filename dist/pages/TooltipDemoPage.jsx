...
import { Tooltip, Popover } from '../components/ui/Tooltip'; // Assuming index.ts is in Tooltip dir
import { MarkdownRenderer } from '../components/ui/MarkdownRenderer'; // Import MarkdownRenderer
import { getTutorialById, getAllTutorialsMetadata } from '../services/TutorialService'; // Import TutorialService
var TooltipDemoPage = function () {
    var _a = useState(null), welcomeTutorial = _a[0], setWelcomeTutorial = _a[1];
    var _b = useState([]), allTutorialsMeta = _b[0], setAllTutorialsMeta = _b[1];
    useEffect(function () {
        getTutorialById('welcome-to-platform').then(function (tutorial) {
            if (tutorial) {
    ...
    return (<div className="container mx-auto p-8 space-y-12">
      ...
      <section>
        ...
        <div className="flex space-x-8 items-center">
          ...
          <Tooltip content="This is a bottom tooltip with more text to see how it wraps." position="bottom">
      ...
      <section>
        ...
        <div className="flex space-x-8 items-start">
          <Popover title="Top Popover" content="This is a popover with a title and some descriptive content. You can put more interactive elements here." position="top">
            <button className="bg-indigo-500 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded">
              Top Popover
            </button>
          </Popover>

          <Popover title="Bottom Popover" content={(<div>
                <p className="mb-2">This popover demonstrates <strong>rich content</strong>.</p>
                <button onClick={function () { return alert('Action from popover!'); }} className="bg-yellow-500 hover:bg-yellow-600 text-black text-sm py-1 px-2 rounded">
                    Click me!
                </button>
              </div>)} position="bottom">
          ...
          <Popover title="Left Popover - Click me" content="This one is on the left." position="left" className="w-64" // Custom width
    >
          ...
          <Popover title="Right Popover - Click me" content="This one is on the right, and has a slightly different offset." position="right" offset={16} // Custom offset
    >
      ...
      <section>
        <h2 className="text-2xl font-semibold mb-4">Popovers (Hover Trigger)</h2>
         <div className="flex space-x-8 items-start">
            <Popover title="Hover Popover (Top)" content="This popover appears on hover." position="top" trigger="hover">
                <span className="bg-gray-200 p-2 rounded cursor-default">Hover for Top Popover</span>
            </Popover>
            <Popover title="Hover Popover (Bottom)" content="This popover also appears on hover, positioned below." position="bottom" trigger="hover">
      ...
      <section>
        ...
        {welcomeTutorial ? (<div>
          ...
          </div>) : (<p>Loading tutorial content...</p>)}

        <div className="mt-6">
            <h3 className="text-xl font-medium mb-2">Other Available Tutorials (Metadata):</h3>
            {allTutorialsMeta.length > 0 ? (<ul className="list-disc pl-5">
                    {allTutorialsMeta.map(function (meta) { return (<li key={meta.id}><strong>{meta.title}</strong> (ID: {meta.id}, Category: {meta.category || 'N/A'})</li>); })}
                </ul>) : (<p>Loading tutorial metadata...</p>)}
        </div>
