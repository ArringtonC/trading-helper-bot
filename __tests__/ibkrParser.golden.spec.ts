...
describe('IBKRActivityStatementParser — Golden Samples', () => {
  ...
  it('extracts cumulativePL from summary row', () => {
    ...
    // Should match the rounded value from the parser
    expect(result.cumulativePL as number).toBeCloseTo(1629.82, 2);
  });
  ...
  it('extracts correct cumulativePL from real IBKR CSV (U5922405_20250218_20250519.csv)', () => {
    const csvPath = path.join(__dirname, '../U5922405_20250218_20250519.csv');
    const csvContent = fs.readFileSync(csvPath, 'utf8');
    const parser = new IBKRActivityStatementParser(csvContent);
    const result = parser.parse();
    expect(result.success).toBe(true);
    // The correct value from the summary row is 1029.319378, should round to 1029.32
    expect(result.cumulativePL as number).toBeCloseTo(1029.32, 2);
    // Simulate UI display
    if (typeof result.cumulativePL !== 'number') throw new Error('cumulativePL is undefined');
    expect(result.cumulativePL.toFixed(2)).toBe('1029.32');
  });
}); 